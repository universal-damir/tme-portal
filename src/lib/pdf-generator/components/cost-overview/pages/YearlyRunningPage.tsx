...
export const YearlyRunningPage: React.FC<PDFComponentProps> = ({ data }) => {
  ...
  const introContent = isIFZAMultiYear 
    ? `This overview outlines the expected recurring costs for maintaining your company license with ${getAuthorityDisplayName()}. These costs will be applied each year after initial ${licenseYears} years. The total cost may vary slightly year to year based on regulatory updates or optional services. Of course, you can also opt for multiple year renewal.`
    : `This overview outlines the expected recurring costs for maintaining your company license with ${getAuthorityDisplayName()}. The total cost may vary slightly year to year based on regulatory updates or optional services.`;
